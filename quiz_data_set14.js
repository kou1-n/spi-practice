...
const SET14_QUESTIONS = [
  ...
  },
  {
    ...
    answer: "C",
    explanation: `<p><strong>正答: C</strong></p><h4>解法ステップ</h4><ol><li>キーワード: 「the intellectual sources for the constitution of the United States」（合衆国憲法の知的源泉）</li><li>本文該当: 「the intellectual root of the American political system goes back to Montesquieu's idea」（アメリカの政治システムの知的ルーツはモンテスキューの思想に遡る）+ 「Montesquieu's political theories have influenced many American political thinkers and lawmakers」</li><li>判定: 本文は「American political system」（政治システム）の知的ルーツと述べているが、「constitution」（憲法）の知的源泉とは直接述べていない → <strong>C</strong></li></ol><h4>解説</h4><p>本文ではモンテスキューの思想がアメリカの「political system」の知的ルーツであり、「political thinkers and lawmakers」に影響を与えたと述べている。しかし、設問は「the constitution of the United States」（合衆国憲法）の知的源泉であると述べている。政治システムへの影響と憲法への影響は関連するが同一ではない。本文は「constitution」という語を使っておらず、モンテスキューの思想が直接的に憲法の知的源泉であるとは明言していない。</p><h4>各選択肢の分析</h4><ul><li>A（正しい）→ 本文は「political system」の知的ルーツとは述べているが、「constitution」の知的源泉とは直接述べていないためAには該当しない。</li><li>B（間違い）→ モンテスキューが政治思想家に影響を与えたことは事実であり、憲法との関連を否定する情報もないためBには該当しない。</li><li><strong>C（判断不能）→ ✓ 正解。</strong> 「political system」と「constitution」は厳密には異なる概念であり、本文から憲法の知的源泉であると断定するには追加情報が必要。</li></ul><div class="tip">時短テク: 本文のキーワード（political system）と設問のキーワード（constitution）のズレに注目。</div>`
  },
  {
    id: 386,
    number: 14,
